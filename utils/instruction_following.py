...
from prompt.instruction_following_prompt import *
import pandas as pd
...
def process(prompt, answer):
    # 假设 mes 和 gpt 已经定义
    message = mes.format(prompt_ins=prompt, answer_ins=answer)
...
def process_data(prompts, answers, max_workers=10):
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 将结果存入新列 '事实理由'
        return pd.DataFrame({'指令跟随理由': results})
...
# 提取括号内的文本到新列 '事实结论'
df['指令跟随结论'] = df['指令跟随理由'].apply(extract_text_in_brackets)
