...
from prompt.math_check_prompt import *
import pandas as pd
...
def process(prompt, answer):
    # 假设 mes 和 gpt 已经定义
    message = system_prompt.format(prompt_math=prompt, answer_math=answer)
    result = o1( messages=message)
    return result
...
def process_data(prompts, answers, max_workers=10):
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 将结果存入新列 '事实理由'
        return pd.DataFrame({'数学理由': results})
...
# 提取括号内的文本到新列 '事实结论'
df['数学结论'] = df['数学理由'].apply(extract_text_in_brackets)
